  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-50">
      ...
      <header className="border-b border-slate-800/80 bg-slate-950/80 backdrop-blur">
        <div className="w-full max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button
              onClick={handleBack}
              className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full bg-slate-900 border border-slate-700 text-xs text-slate-300 hover:border-slate-500"
            >
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                Vista individual desde <code>assets_public</code>.
              </p>
            </div>
          </div>
      ...
      <main className="w-full flex justify-center px-4 py-8 md:py-10">
        {loading ? (
          <div className="text-slate-400 text-sm py-16">
            Cargando activo...
          </div>
        ) : !asset ? (
          <div className="text-slate-400 text-sm py-16">
          ...
          <div className="w-full max-w-6xl grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
            {/* COLUMNA IZQUIERDA */}
            <section className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-2xl shadow-black/40 space-y-5">
              {/* Imagen */}
              {asset.image_url ? (
                <div className="relative w-full h-80 rounded-xl overflow-hidden border border-slate-800 bg-slate-900 flex items-center justify-center">
                  <img
                    src={asset.image_url}
                    alt={asset.name}
                    className="max-h-full max-w-full object-contain"
                  />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-950/80 to-transparent px-4 py-3 flex items-center justify-between">
                ...
                <div className="w-full h-80 rounded-xl border border-slate-800 bg-slate-900/60 flex flex-col items-center justify-center text-slate-500 text-sm gap-2">
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div>
                  <p className="text-[11px] text-slate-500 uppercase mb-1">
                    Código
                  </p>
                  <p className="font-mono text-sm text-slate-300">
                    {asset.code}
                  </p>
                  <h2 className="mt-3 text-xl md:text-2xl font-semibold text-slate-50">
                    {asset.name}
                  </h2>
                  {asset.description && (
              ...
              <div className="mt-2 border-t border-slate-800 pt-4 text-[11px] text-slate-500 space-y-1.5">
            ...
            <aside className="space-y-4">
              ...
              <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl shadow-black/40 space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="inline-flex items-center justify-between w-full px-3 py-2 rounded-xl border border-slate-700 bg-slate-950/60 text-xs text-slate-100 hover:border-emerald-400 hover:bg-slate-900 transition"
                  >
                  ...
                  <button
                    type="button"
                    onClick={handlePrint}
                    className="inline-flex items-center justify-between w-full px-3 py-2 rounded-xl border border-slate-700 bg-slate-950/40 text-xs text-slate-100 hover:border-slate-500 hover:bg-slate-900 transition"
                  >
                    <span className="flex items-center gap-2">
                  ...
                  <Link
                    to="/"
                    className="inline-flex items-center justify-center w-full px-3 py-2 rounded-xl border border-slate-800 bg-slate-900/60 text-xs text-slate-200 hover:bg-slate-800 hover:border-slate-600 transition"
                  >
                    Volver al inventario
                  </Link>
                </div>
...
function DetailField({ label, value, mono, icon }) {
  return (
    <div className="space-y-1">
      <p className="text-[11px] text-slate-500 uppercase flex items-center gap-1.5">
        {icon}
        {label}
      </p>
      {typeof value === "string" ? (
